class Categorizer
  ...
  FundTags = %w(Main Parallel).map(&:downcase)
  VisibilityTags = %w(LPs Advisors Entities).map(&:downcase).map(&:singularize)
  ...
  def fund_tag
    @path.each do |p|
      p = p.downcase
      return p unless FundTags.index(p).nil?
    end
  ...
  def visibility_tag
    @path.each do |p|
      ...
      return p unless VisibilityTags.index(p).nil?
    end
  ...
  def self.extract_filing_date(file)
    ...
    path = extract_path_entries(file)
    filing_year = nil
    filing_month = nil
