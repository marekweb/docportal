class BoxAdapter
  ...
  def self.refresh_token(access_token, refresh_token)
    box_session = create_session(access_token)
  ...
  def self.sync_folder_by_name(name)
    box_sync = create_box_sync_client
